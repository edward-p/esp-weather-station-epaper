...
void WaveShare_EPD::DrawUnicodeChar(byte x, int16_t y, byte width, byte height, unsigned char *code)
{
	...
	//Serial.println(offset);
	if (offset < 0xff * sizeofsinglechar && FontIndex < 10) drawXbm(x, y, width, height, (unsigned char *)zi); else drawXbm(x, y, width, height, (unsigned char *)zi);
	SPIFFS.end();
}
	...
	while (i < strlength) {
		...
		if (offset < 0xff * sizeofsinglechar) {
			...
			if ((y + ymove + width) >= yDot - 1) {
				xmove += height + 1; ymove = 0; CurrentCursor = 0;
			}
		}else if (fontscale == 2) {
			DrawUnicodeChar(x + xmove, y + ymove, width, height, (unsigned char *)code + i);
			ymove += width * 2;
			if ((y + ymove + width * 2) >= yDot - 1) {
				xmove += height + 1; ymove = 0; CurrentCursor = 0;
			}
		}else    {
			DrawUnicodeChar(x + xmove, y + ymove, width, height, (unsigned char *)code + i);
...
void WaveShare_EPD::drawXbm(int16_t xMove, int16_t yMove, int16_t width, int16_t height, unsigned char *xbm)
{
	...
	for (int16_t x = 0; x < width; x++) {
		for (int16_t y = 0; y < height; y++ ) {
			...
			if (((data & 0x80) >> 7)) {
				if (fontscale == 1) {
					SetPixel(xMove + y, yMove + x); CurrentCursor = x;
				}
				if (fontscale == 2) {
					SetPixel(xMove + y * 2, yMove + x * 2); SetPixel(xMove + y * 2 + 1, yMove + x * 2); SetPixel(xMove + y * 2, yMove + x * 2 + 1); SetPixel(xMove + y * 2 + 1, yMove + x * 2 + 1);
				}
			}
		}
	}
